...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    entriesList.innerHTML = entries
      .map((entry, i) => {
        ...
        <div class="entry-card" data-idx="${i}">
          <div class="entry-icon" style="background:transparent">
            ${
              faviconUrl
  ...
  saveForm.addEventListener("submit", async (e) => {
    ...
    if (res.error) {
      ...
      saveSuccess.textContent = res.updated
        ? "✅ Password updated in vault!"
  ...
  try {
    ...
    if (tab?.url) {
      ...
      document.getElementById("save-name").value =
        host.split(".").slice(0, -1).join(".") || host;
    }
  } catch {
    /* ignore */
  }
